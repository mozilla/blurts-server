...
test.describe("Breach Resolution", () => {
  test("Verify breach resolution loads", async ({
    ...
    const firstBreach = dataBreachPage.breachRows.first();
    ...
    const firstBreachResolveListItems = page
      .locator(".resolve-list")
      .first()
      .locator(".resolve-list-item");
    ...
    const updatedResolvedBreachTab = page.getByText(/1Resolved breaches/);
    expect(updatedResolvedBreachTab).toBeTruthy();

    await testInfo.attach(
      `${process.env.E2E_TEST_ENV}-breach-resolve-monitor-dashboard.png`,
      {
...
test.describe("Breaches Dashboard - Headers", () => {
  ...
  test(" Verify that the user can navigate through the Monitor dashboard", async ({
    ...
    // get expected links
    const links = dataBreachPage.dashboardLinks();
