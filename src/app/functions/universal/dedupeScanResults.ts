...
function removeIrrelevantResults(scanResults: MoscaryData["ScanResult"][]) {
  return scanResults.filter((scanResult, currentIndex) => {
    // Only include one scan results per result URL:
    if (scanResult.link) {
      const previousDuplicateResultIndex = scanResults.findIndex(
        (result) => result.link === scanResult.link,
      );
      if (
        previousDuplicateResultIndex !== -1 &&
        previousDuplicateResultIndex < currentIndex
      ) {
        return false;
      }
    }
