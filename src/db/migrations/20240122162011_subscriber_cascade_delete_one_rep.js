...
export async function up(knex) {
  await knex.schema.alterTable("email_notifications", table => {
    table.dropForeign("subscriber_id").foreign("subscriber_id").references("subscribers.id").onUpdate("CASCADE").onDelete("CASCADE")
    table.dropForeign("breach_id").foreign("breach_id").references("breaches.id").onUpdate("CASCADE").onDelete("CASCADE")
  });
  await knex.schema.alterTable("onerep_scan_results", table => {
    table.dropForeign("onerep_scan_id").foreign("onerep_scan_id").references("onerep_scans.onerep_scan_id").onUpdate("CASCADE").onDelete("CASCADE")
  })
  await knex.schema.alterTable("onerep_scans", table => {
    table.dropForeign("onerep_profile_id").foreign("onerep_profile_id").references("subscribers.onerep_profile_id").onUpdate("CASCADE").onDelete("CASCADE")
  })
