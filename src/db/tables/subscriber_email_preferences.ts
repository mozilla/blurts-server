...
import { getSubscriberById, setAllEmailsToPrimary } from "./subscribers";
...
// Not covered by tests; mostly side-effects. See test-coverage.md#mock-heavy
/* c8 ignore start */
async function unsubscribeBreachAlertsForUnsubscribeToken(
  unsubscribeToken: string,
) {
  try {
    const sub = await getEmailPreferenceForUnsubscribeToken(unsubscribeToken);

    if (typeof sub?.subscriber_id === "number") {
      const subscriber = await getSubscriberById(sub.subscriber_id);
      if (!subscriber) {
        logger.error(
          "unsubscribe_breach_alerts_for_unsubscribe_token_subscriber_not_found",
          { subscriberId: sub.subscriber_id },
        );
        captureMessage(
          `unsubscribe_breach_alerts_for_unsubscribe_token_subscriber_not_found: ${sub.subscriber_id}`,
        );
        return;
      }
    ...
    logger.error("error_unsubscribe_breach_alerts_for_unsubscribe_token", {
      message: (e as Error).message,
      stack_trace: (e as Error).stack,
      unsubscribeToken,
    });
...
export {
  ...
  unsubscribeBreachAlertsForUnsubscribeToken,
};
