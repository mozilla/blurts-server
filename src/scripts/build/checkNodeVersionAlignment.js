...
const ghaVersions = ghaWorkflows.flatMap(([filename, contents]) => {
  return contents.split("\n")
    ...
    .map(line => [filename, line.split("node-version: ")[1].replaceAll("'", "")]);
});
...
const checkVersion = (source, version) => {
  ...
  if (getMinorOnly(voltaVersion) !== getMinorOnly(version)) {
    console.error(`Error: version [${version}] in ${source} does not match the Volta version ([${voltaVersion}]).`)
    process.exit(1);
  }
};

checkVersion("the package.json engines", packageJson.engines.node);
checkVersion("the package-lock.json engines", lockFile.packages[""].engines.node);
