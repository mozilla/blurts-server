...
import { uploadToS3, checkS3ObjectExists } from "../../utils/s3.js";
...
export async function getBreachIcons(breaches: HibpGetBreachesResponse) {
  for (const breach of breaches) {
    ...
    try {
      ...
      const exists = await checkS3ObjectExists(logoFilename);
      ...
      if (res.status !== 200) {
        logger.warning("Logo does not exist or is inaccessible", {
          breachName,
          breachDomain,
          iconUri,
          status: res.status,
      ...
      await uploadToS3(logoFilename, logoBuffer);
