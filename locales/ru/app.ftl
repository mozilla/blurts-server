...
sensitive-sites-copy = { -product-name } показывает аккаунты, связанные с этими типами утечек, только после подтверждения адреса электронной почты. Это означает, что вы — единственный человек, который может увидеть, была ли ваша информация затронута этой утечкой данных (если только кто-то другой не имеет доступа к вашей учётной записи электронной почты).
...
what-is-data-agg-blurb = Агрегаторы или брокеры данных собирают информацию из общедоступных записей и других источников, включая данные, купленные у компаний. Они собирают эту информацию для создания подробных профилей потребителей, которые продаются компаниям в маркетинговых целях. Хакеры могут использовать эти данные для профилирования, паролей или других форм мошенничества. Вы можете не узнать некоторые из этих компаний, потому что агрегаторы данных часто собирают информацию о людях без того, чтобы они создавали аккаунт или регистрировались в службе.
