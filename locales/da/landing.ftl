# Landing page


## Hero

get-started = Kom i gang

## Why use Firefox Monitor?

why-use-monitor = Hvorfor bruge { -brand-fx-monitor }?
identifying-breaches = Det er vigtigt for dit privatliv på nettet, at du identificerer og afhjælper datalæk.
protect-account = Beskyt dine konti
protect-account-prevent-hackers = Undgå at hackere får adgang til dine konti med lækkede adgangskoder.
prevent-fraud = Forebyg bedrageri og cyberkriminalitet
prevent-fraud-keep-info = Beskyt dig mod hackere, der forsøger at stjæle din identitet og begå forbrydelser.
get-alerts = Få advarsler om nye datalæk
get-alerts-find-out = Få besked, hvis det bliver opdaget, at dine personlige oplysninger er blevet kompromitteret i en ny datalæk.

## Here’s how it works

how-it-works = Sådan fungerer det
check-for-breaches = Undersøg for datalæk
check-for-breaches-we-search = Vi vil gennemsøge alle kendte datalæk siden 2007 for at se, om dine oplysninger er blevet kompromitteret.
protect-accounts = Beskyt dine konti
protect-accounts-clear-steps = Vi vejleder dig trin for trin, hvis du bliver udsat for en datalæk. Så ved du, hvordan du skal håndtere det.
alerts-for-breaches = Få advarsler om nye datalæk
alerts-for-breaches-monitor-new = Vi holder løbende øje med nye datalæk og giver dig besked, hvis dine oplysninger er omfattet.

## Your privacy is safe with us

safe-with-us = Vi beskytter dit privatliv på nettet
parent-company = Hos { -brand-Mozilla }, virksomheden bag { -brand-firefox } og { -brand-fx-monitor }, mener vi, at beskyttelse af privatlivet og sikkerhed på nettet er grundlæggende. Det er ikke noget, der skal tages let på.
our-mission = Vores mission er at skabe et bedre internet. Et internet, hvor folk kan forme deres egne oplevelser, være sikre og have kontrol. Vi er ejet af nonprofit-organisationen { -brand-mozilla-foundation }, og derfor kan vi vægte individuelle og samfundsmæssige behov højere end profit.
learn-more-mission = Læs mere om vores mission

## Top questions about Firefox Monitor

top-questions-about-monitor = De almindeligste spørgsmål om { -brand-fx-monitor }
see-all-faq = Se alle ofte stillede spørgsmål
# question and answer
what-is-breach = Hvad er en datalæk helt præcist?
when-info-exposed = En datalæk er, når personlige eller private oplysninger bliver afsløret, stjålet eller kopieret uden tilladelse. Denne type af sikkerhedshændelser kan skyldes cyberangreb på websteder, apps eller databaser, hvor folks personlige oplysninger er lagret. En datalæk kan også ske tilfældigt, fx hvis en persons login-oplysninger ved et uheld bliver publiceret offentligt.
# question and answer
what-do-i-do = Jeg har lige fundet ud af, at jeg er omfattet af en datalæk. Hvad gør jeg?
visit-monitor-to-learn = Besøg { -brand-fx-monitor } for at se, hvad du skal gøre, hvis du er blevet ramt af en datalæk. Hackere udnytter, at folk genbruger deres adgangskoder, så det er vigtigt, at du har stærke og unikke adgangskoder til alle dine konti. Opbevar dine adgangskoder et sikkert sted, som kun du har adgang til - fx samme sted, du opbevarer dine vigtige dokumenter, eller i et særligt program til at håndtere adgangskoder.
# question and answer
what-gets-exposed = Hvilke oplysninger bliver kompromitteret i datalæk?
depends-on-hackers = Forskellige typer af oplysninger kan blive afsløret i forskellige datalæk. Det kommer helt an på, hvilke data hackerne kan få fat i. I mange datalæk bliver mailadresser og adgangskoder afsløret, i andre er det følsomme data som fx oplysninger om betalingskort, pasnumre eller personnumre.

## See if you’ve been in a data breach

see-if-data-breach = Se om du er blevet ramt af en datalæk
hibp-footer-attribution = Information om datalæk stammer fra <b>{ -brand-HIBP }</b>
