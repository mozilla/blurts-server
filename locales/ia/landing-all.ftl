...
landing-all-hero-emailform-submit-sign-up-label = Accede pro un scansion gratuite
...
landing-all-value-prop-fix-exposures-description = Nostre mission es de re-poner le controlo de tu datos personal in tu manos.  Nos te adjutara a resolver violationes de datos e mantener private tu info, e nos <privacy_link>respectara tu confidentialitate</privacy_link> in le processo.
...
landing-all-quote = <data_breaches>Violationes de datos</data_breaches> eveni cata 11 minutas, exponente tu informationes private, ma non te preoccupa, nos pote adjutar.
...
landing-all-social-proof-title =
    { $num_users ->
        [one] Digne de fide per { $num_users } million de personas per tote le mundo
       *[other] Digne de fide per { $num_users } milliones de personas per tote le mundo
    }
# Variables
# $num_countries is the number of countries available.
landing-all-social-proof-description =
    { $num_countries ->
        [one] Desde le 2018 nos adjuta personas in { $num_countries } pais a proteger lor proprie datos in caso de exposition.
       *[other] Desde le 2018 nos adjuta personas in { $num_countries } paises a proteger lor proprie datos in caso de exposition.
    }
