fx-monitor = { -product-name }
pocket = { -brand-pocket }
fx-send = { -brand-send }
fx-lockwise = { -brand-lockwise }

## If possible, keep "Firefox Browser" in English, but feel free 
## to translate browser if that doesn't work for your language.

fx-desktop = Nettleseren { -brand-name } for datamaskiner
fx-mobile = Nettleseren { -brand-name } for mobil



# Aria label for the Bento button
bento-button-title = Apper og tjenester fra { -brand-name }
# Bento headline
fx-makes-tech = { -brand-name } er teknologi som jobber for å beskytte ditt personvern på nettet.
# A link at the bottom of the Bento menu
made-by-mozilla = Laget av { -brand-Mozilla }
# Aria label for button that closes the Bento menu
mobile-close-bento-button-title = Lukk meny
