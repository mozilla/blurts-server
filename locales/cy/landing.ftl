# Landing page


## Hero

find-out-if-breached = Darganfyddwch a ydych wedi bod mewn tor-data
stay-safe-with-tool = Cadwch yn ddiogel gyda theclyn syml ond pwerus sy'n olrhain a yw eich manylion personol wedi'i datgelu mewn achos o dor-data. Byddwn yn eich helpu i ddeall beth i'w wneud nesaf ac yn monitro unrhyw doriadau newydd yn barhaus — y cyfan heb gost i chi.
get-started = Cychwyn arni

## Why use Firefox Monitor?

why-use-monitor = Pam defnyddio { -brand-fx-monitor }?
identifying-breaches = Mae adnabod a datrys achosion o dor-data yn gam pwysig i sicrhau eich preifatrwydd ar-lein.
protect-account = Diogelu eich cyfrifon
protect-account-prevent-hackers = Rhwystro hacwyr rhag mynd i mewn i'ch cyfrifon gyda chyfrineiriau o dor-data.
prevent-fraud = Rhwystro twyll a seiberdroseddau
prevent-fraud-keep-info = Cadw'ch manylion rhag hacwyr sydd am eich dynwared a chyflawni troseddau.
get-alerts = Derbyn rhybuddion am dor-data newydd
get-alerts-find-out = Darganfod pryd bynnag y caiff eich manylion personol ei beryglu mewn tor-data hysbys newydd.

## Here’s how it works

how-it-works = Dyma sut mae'n gweithio:
check-for-breaches = Gwirio am dor-data
check-for-breaches-we-search = Byddwn yn chwilio’r holl dor-data hysbys ers 2007 i weld a oedd eich manylion wedi’i beryglu.
protect-accounts = Diogelu eich cyfrifon
protect-accounts-clear-steps = Byddwn yn rhoi camau clir i chi ar beth i'w wneud nesaf ar gyfer unrhyw dor-data rydych wedi bod yn rhan ohono.
alerts-for-breaches = Derbyn rhybuddion am dor-data newydd
alerts-for-breaches-monitor-new = Byddwn yn monitro’n barhaus am dor-data newydd ac yn rhoi gwybod i chi os yw hyn wedi’ch effeithio chi.

## Your privacy is safe with us

safe-with-us = Mae eich preifatrwydd yn ddiogel gyda ni
parent-company = Yn { -brand-Mozilla }, rhiant-gwmni { -brand-firefox } a { -brand-fx-monitor }, rydym yn credu fod diogelwch a phreifatrwydd unigolion ar y rhyngrwyd yn hanfodol ac na ddylid ei drin fel rhywbeth dewisol.
our-mission = Ein cenhadaeth yw adeiladu rhyngrwyd gwell - un lle gall pobl lunio eu profiadau eu hunain, teimlo eu bod wedi'u grymuso a'u cadw'n ddiogel. Mae { -brand-mozilla-foundation }, sefydliad dim-er-elw, yn ein galluogi ni i roi budd unigol a chyhoeddus o flaen elw.
learn-more-mission = Dysgu rhagor am ein cenhadaeth

## Top questions about Firefox Monitor

top-questions-about-monitor = Y prif gwestiynau am { -brand-fx-monitor }
see-all-faq = Gweld yr holl gwestiynau cyffredin
# question and answer
what-is-breach = Beth yn union yw tor-data?
when-info-exposed = Mae tor-data'n digwydd pan fydd manylion personol neu breifat yn cael ei ddatgelu, ei dwyn neu ei gopïo heb ganiatâd. Gall y digwyddiadau diogelwch hyn fod o ganlyniad i ymosodiadau seiber ar wefannau, apiau neu unrhyw gronfa ddata lle mae manylion personol pobl yn cael eu cadw. Gall tor-data ddigwydd ar ddamwain hefyd, fel pan mae manylion mewngofnodi rhywun yn cael eu postio'n gyhoeddus yn ddamweiniol.
# question and answer
what-do-i-do = Rwyf newydd ddeall fy mod i mewn tor-data. Beth ddylwn i ei wneud nesaf?
visit-monitor-to-learn = Ewch i { -brand-fx-monitor } i ddysgu beth i'w wneud ar ôl tor-data. Mae hacwyr yn dibynnu ar bobl yn ailddefnyddio cyfrineiriau, felly mae'n bwysig creu cyfrineiriau cryf, unigryw ar gyfer eich holl gyfrifon. Cadwch eich cyfrineiriau mewn man diogel, a dim ond  chi sydd â mynediad iddo; gall hwn fod yr un man lle rydych yn storio dogfennau pwysig neu reolwr cyfrinair.
# question and answer
what-gets-exposed = Pa wybodaeth sy'n cael ei datgelu mewn achosion o dorri data?
depends-on-hackers = Nid yw pob tor-data'n amlygu'r un wybodaeth. Mae'n dibynnu ar yr hyn y gall hacwyr gael mynediad ato. Mae llawer o achosion o dor-data yn amlygu cyfeiriadau e-bost a chyfrineiriau. Mae eraill yn datgelu manylion mwy sensitif fel rhifau cardiau credyd, rhifau pasbort a rhifau nawdd cymdeithasol.

## See if you’ve been in a data breach

see-if-data-breach = Gweld a ydych wedi ymddangos mewn tor-data.
