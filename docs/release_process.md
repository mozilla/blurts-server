...
3. Generate release notes from the time since the last released tag (see https://monitor.mozilla.org/__version__ for the commit hash)
4. Copy and Paste the release notes in the engineering slack channel so the team is aware, ideally omitting Dependabot/l10n updates
...
- With your pairing partner, create a new tag from the hotfix commit and mark it as the latest release
  - Use the date-formatted tag. Add an additional numeric suffix if there is already a tag for that day (e.g. `2024.02.27.1`)
