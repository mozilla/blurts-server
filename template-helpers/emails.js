...
function emailBreachStats(args) {
  ...
  const emailBreachStats = {
    numBreaches: {
      ...
      statTitle: LocaleUtils.fluentFormat(
        locales,
        "known-data-breaches-exposed",
        { breaches: userBreaches.length }
      ),
    },
    numPasswords: {
      statNumber: numPasswordsExposed,
      statTitle: LocaleUtils.fluentFormat(locales, "passwords-exposed", {
...
function getUnsafeBreachesForEmailReport(args) {
  const locales = args.data.root.supportedLocales;
  const foundBreaches = JSON.parse(
    JSON.stringify(args.data.root.unsafeBreachesForEmail)
  );
...
function getEmailHeader(args) {
  ...
  if (userBreaches.length === 0) {
    return LocaleUtils.fluentFormat(locales, "email-no-breaches-hl", {
      userEmail: boldVioletText(breachedEmail, true),
    });
  }
...
function makeFaqLink(target, campaign) {
  const url = new URL(
    `https://support.mozilla.org/kb/firefox-monitor-faq${target}`
  );
  const utmParameters = {
    utm_source: "fx-monitor",
    utm_medium: "email",
...
function getBreachAlertFaqs(args) {
  ...
  const faqs = [
    {
      ...
      stringDescription:
        "I don’t recognize one of these companies or websites. Why am I in this breach?",
      href: makeFaqLink(
        "#w_i-donaot-recognize-this-company-or-website-why-am-i-receiving-notifications-about-this-breach",
        "faq1"
      ),
    },
    {
      ...
      href: makeFaqLink(
        "#w_do-i-need-to-do-anything-if-a-breach-happened-years-ago-or-in-an-old-account",
        "faq2"
      ),
    },
    {
      ...
      href: makeFaqLink(
        "#w_i-just-found-out-im-in-a-data-breach-what-do-i-do-next",
        "faq3"
      ),
    },
  ];
  ...
  if (args.data.root.breachAlert && args.data.root.breachAlert.IsSensitive) {
    faqs.push({
      ...
      href: makeFaqLink(
        "#w_how-does-firefox-monitor-treat-sensitive-sites",
        "faq4"
      ),
    });
  }
...
function getReportHeader(args) {
  ...
  for (const stringId in reportHeader.strings) {
    const newString = LocaleUtils.fluentFormat(
      locales,
      reportHeader.strings[stringId]
    );
...
function getBreachSummaryHeadline(args) {
  const locales = args.data.root.supportedLocales;
  const breachedEmail = args.data.root.breachedEmail;
  return LocaleUtils.fluentFormat(locales, "email-breach-summary-for-email", {
    userEmail: boldVioletText(breachedEmail),
  });
}
...
function showFaqs(args) {
  ...
  if (
    args.data.root.breachAlert ||
    (args.data.root.unsafeBreachesForEmail &&
      args.data.root.unsafeBreachesForEmail.length > 0)
  ) {
    return args.fn();
  }
}
...
//MH TODO: Could remove if not sending separate data removal specific email on signup
function getRemovalEmailFooterCopy(args) {
  const locales = args.data.root.supportedLocales;
...
module.exports = {
  ...
  showFaqs,
  showProducts,
  getRemovalEmailFooterCopy, //DATA REMOVAL SPECIFIC
};
