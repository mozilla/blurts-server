...
```
npx next typegen && npx tsc -p tsconfig.json --noEmit
```

Note: The repo has multiple tsconfig files (`tsconfig.json` and `tsconfig.cronjobs.json`). CI uses `-p tsconfig.json`.
...
node src/scripts/build/checkGithubActionsBestPractices.js
```

`npm run lint` runs in sequence: `stylelint '**/*.scss'`, `prettier --check './src'`, `eslint --max-warnings=0 .`, `next typegen`, `tsc -p tsconfig.json --noEmit`, and `validate-nimbus`. The two `node` scripts above run as separate CI steps after `npm run lint`.
...
- Do not modify CI configuration files, unless the dependency update directly breaks a CI check definition (e.g., a renamed binary or changed CLI flag in a workflow step). In that case, limit changes to the affected step only.
